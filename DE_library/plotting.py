...
def time_series(T,
    ...
    for sp, X_ in enumerate(X):
        ...
        for i in range(len(X_)-2):
            ...
            c = colors[i] if len(colors) > 1 and not isinstance(
                colors, str) else colors

            ax.plot(T[i:i+2], X_[i:i+2], style, c=c,
...
def trajectories(X,
    ...
    if dim == 2:
        ...
        if '-' in style:
            ax.plot(X[:, 0], X[:, 1], c=c, linewidth=lw,
                    markersize=ms, alpha=al)
        if '>' in style:
        ...
        if '-' in style:
            ax.plot(X[:, 0], X[:, 1], X[:, 2], c=c,
                    linewidth=lw, markersize=ms, alpha=al)
        if '>' in style:
            arrow_prop_dict = dict(
                mutation_scale=arrowhead, arrowstyle='-|>', color=c, alpha=al, lw=lw)
            skip = (slice(None, None, arrow_spacing), slice(None))
            X, V = X[skip], V[skip]
            for j in range(X.shape[0]):
...
def set_colors(color, cmap=plt.cm.coolwarm):
    ...
    if isinstance(color[0], (float, np.floating)):
        if (color >= 0).all():
            ...
            norm = plt.cm.colors.Normalize(-np.max(np.abs(color)),
                                           np.max(np.abs(color)))
